...
const Navbar = () => {
    ...
    return (
        <>
            ...
            <AnimatePresence>
                {isMenuOpen && (
                    <motion.div initial={{ opacity: 0, y: -50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -50 }} className="fixed top-0 left-0 w-full h-screen bg-white z-30 pt-24">
                        <div className="flex flex-col items-center justify-center h-full space-y-8">{navLinks.map(link => ( <a key={link} href={`#${link.toLowerCase()}`} className="text-2xl text-slate-700 hover:text-blue-600" onClick={() => setIsMenuOpen(false)}>{link}</a> ))}</div>
                    </motion.div>
                )}
            </AnimatePresence>
        </>
